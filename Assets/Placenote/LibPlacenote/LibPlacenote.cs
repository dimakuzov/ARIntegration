...
public class LibPlacenote : MonoBehaviour
{
    ...
    //Added by Cliff Lin>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    //This is the indicator for whether 'mapping' is in the extending mode.
    private bool extendingMode = false;

    /// <summary>
    /// This method indicates whether user is performing actually mapping.
    /// </summary>
    /// <returns></returns>
    public bool IsPerformingMapping()
    {
        MappingMode mode = this.GetMode();
        MappingStatus status = this.GetStatus();

        

        return mode == MappingMode.MAPPING && status == MappingStatus.RUNNING
            || (mode == MappingMode.LOCALIZING && this.extendingMode && status == MappingStatus.RUNNING);

        //return this.GetMode() == MappingMode.MAPPING && this.GetStatus() == MappingStatus.RUNNING
        //    || (this.GetMode() == MappingMode.LOCALIZING && this.extendingMode && this.GetStatus() == MappingStatus.RUNNING);
    ...
    private void DownloadThumbnail(string mapId, Action<Texture2D> thumbnailCb)
    {
        ...
        LibPlacenote.Instance.SyncLocalizationThumbnail(mapId, thumbnailPath,
            (completed, faulted, progress) =>
            {
                ...
                thumbnailCb(mThumbnailTexture);
                //Added by Cliff Lin>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                SocialBeeAR.DebugMessageManager.Instance.PrintDebugMessage(String.Format("Downloaded localization thumbnail {0} {1}",
    ...
    /// </returns>
    public PNFeaturePointUnity[] GetMap(int measCountThreshold = 2)
    {
        ...
        List<PNFeaturePointUnity> mapFiltered = new List<PNFeaturePointUnity>();
        foreach (var lm in map)
        {
    ...
    /// </returns>
    public PNFeaturePointUnity[] GetTrackedFeatures(int measCountThreshold = 2)
    {
        ;
        PNFeaturePointUnity[] trackedLms = new PNFeaturePointUnity[1];

#if !UNITY_EDITOR
		int lmSize = PNGetTrackedLandmarks (trackedLms, 0);
		...
		Array.Resize (ref trackedLms, lmSize);
		PNGetTrackedLandmarks (trackedLms, lmSize);
#endif
        ...
        List<PNFeaturePointUnity> filtered = new List<PNFeaturePointUnity>();
        foreach (var lm in trackedLms)
        {
            if (lm.measCount > measCountThreshold)
            {
                // add to point cloud array
